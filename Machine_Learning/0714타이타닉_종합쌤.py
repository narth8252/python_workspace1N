...
# -------------------------------------------------
path = r"C:\Users\Admin\Documents\GitHub\python_workspace1N\Data_Analysis_2507\data\titanic"
...
#6. 학습에 사용해서, RandomForest로 훈련시키고, 훈련 정확도를 출력하는 과정
#Survived가 젤앞에 있음: 전제는: train의 첫 번째 열이 **Survived (정답 레이블)**라는 가정
X = train.iloc[:, 1:] #데이터에서 첫 번째 열을 제외한 나머지 전부 (즉, 입력 특성들)
y = train.iloc[:, 0] #데이터의 첫 번째 열, 즉 타겟값(보통 Survived)
print(X.shape)
print(y.shape)
