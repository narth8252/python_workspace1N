# 250820 허깅페이스 https://wikidocs.net/280976#1-transformers
...
# 코드에 직접 입력하는 경우 보안에 유의하세요. #쌤토큰이니 내꺼 따로 받아라.
hf_token = ""
...
try:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        token=hf_token,
        torch_dtype=torch.float32,         # CPU 안전 옵션
        low_cpu_mem_usage=True             # 메모리 사용 감소에 도움
    )
    model.to(device)
    model.eval()
    ...
    """
    # 4. 텍스트 생성
    prompt_text = input("질문을 하세요: ").strip() or "대한민국의 수도는 어디야?"
    print(f"\n입력 프롬프트: {prompt_text}")
