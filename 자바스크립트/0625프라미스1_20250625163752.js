...
/*
설명
ㅌsync 함수는 항상 Promise를 반환합니다.
값을 바로 출력하려면 .then() 또는 await를 사용해야 실제 결과를 볼 수 있습니다.
정리
Promise는 비동기 작업의 완료/실패를 나타내는 객체로, pending, fulfilled, rejected 세 가지 상태를 가집니다.
.then(), .catch(), .finally()를 통해 결과 처리 및 에러 처리가 가능합니다.
async 함수는 항상 Promise를 반환하며, 값을 얻으려면 .then()이나 await를 사용해야 합니다.
코드의 변수명, for문 조건 등 오타를 주의해야 하며, Promise와 async/await의 동작 원리를 이해하면 비동기 코드를 효율적으로 작성할 수 있습니다.
*/