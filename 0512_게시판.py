...
# #추가기능1.회원가입 -1)Uesr클래스 수정
# class User:
#     def __init__(self):
#         self.users = {}  # 아이디:비밀번호 저장
#         self.user_id = None  # 현재 로그인한 사용자
...
#         input_id = input("아이디 : ")
#         input_pw = input("비밀번호 : ")
#         if input_id in self.users and self.users[input_id] == input_pw:
...
#         while True:
#             print(f"--- 게시판 ({user.user_id}) ---")
#                                 #user만으로는 로그인유저 정보 처리안됨	→ user.user_id 저장/검사방식으로 수정
#             print("1. 글 쓰기")
#             print("2. 글 목록")
#             print("3. 이전 메뉴로")
#             post_choice = input("선택 (번호로): ")
...
#                 board.write_post(user.user_id) 
#                 #user만쓰면 왜 안돼?
# # user	        User클래스의 객체자체 (ex.User() 로 만든 인스턴스 전체)
# # user.user_id	그객체 안에있는 로그인한 user_id값(문자열)
# # user = User()  User클래스의 인스턴스 전체
# # user.user_id = "kim" ← 로그인한 실사용자의 id값
# #board.write_post(user) : User객체전체를 넘기게 되는데,write_post()함수는 이렇게 받도록 만들어져있지않음.
#                     #함수내에서 user_id라는 문자열이 필요하지, User객체전체는 필요X
#         #def write_post(self, user_id):  # user_id는 문자열로 기대됨
#         #user 는 "사용자전체", user.user_id는 사용자id
#         #필요시, user 전체를 넘기고, 함수안에서 user.user_id를 꺼내쓰도록 구조변경가능.

#             elif post_choice == "2":
#                 board.list_post()
#             elif post_choice == "3":
#                 break 
#         #"3.이전메뉴 누르면 메인메뉴(로그인/글쓰기/나가기)로 돌아가게? 번호를 문자열""로 감싸라
...
#     elif choice == "3":
#         print("종료")
#         break # return 함수밖에서 사용→ break 수정
#     else:
#         print("번호를 잘못 선택.\n")
