...
# SVM모델을 스케일링전 원본데이터로 학습후 결과확인
from sklearn.svm import SVC
model = SVC()   # 분류
model.fit(X_train, y_train)
print("-------- 스케일링 안한 서포트벡터머신 ----------")
print("훈련셋", model.score(X_train, y_train))
print("테스트셋", model.score(X_test, y_test)) #정확도0.629로 매우낮음

print()
# ✨3. 데이터 스케일링(가장중요), SVM모델성능향상위한 핵심과정
# 각특성(feature)의 값범위가 제각각이므로 일정한범위로 맞춰주는 스케일링

# 4.스케일링후 모델성능확인
# train_test_split 함수에 들어가는 데이터가 원본 X가 아닌 스케일링이 완료된 X_scaled
print("-------- 스케일링된 서포트벡터머신 ----------")
...
###################################################최종
# ... (생략: 데이터 준비와 스케일링, train_test_split 원본 데이터)

# 1) 스케일링 전 SVM 학습 및 평가
model = SVC()
model.fit(X_train, y_train)
print("-------- 스케일링 안한 서포트벡터머신 ----------")
print("훈련셋", model.score(X_train, y_train))
...
# 2) 스케일링 후 train_test_split
X_train_scaled, X_test_scaled, y_train_scaled, y_test_scaled = train_test_split(
    X_scaled, y, random_state=0)

# 3) 스케일링 후 SVM은 새로운 모델 객체로 학습 및 평가
model_scaled = SVC()
model_scaled.fit(X_train_scaled, y_train_scaled)
print("-------- 스케일링된 서포트벡터머신 ----------")
print("훈련셋", model_scaled.score(X_train_scaled, y_train_scaled))
print("테스트셋", model_scaled.score(X_test_scaled, y_test_scaled))
