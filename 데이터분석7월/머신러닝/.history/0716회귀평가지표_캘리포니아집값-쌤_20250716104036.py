...
# •LinearRegression(): 선형 회귀 모델 객체를 생성합니다.
# •model.fit(X_train_scaled, y_train): 스케일링된 훈련 데이터와 해당 데이터의 정답(주택 가격) 을 가지고 모델을 학습시킵니다.
# •model.predict(X_test_scaled): 학습된 모델을 사용하여 스케일링된 테스트 데이터의 주택 가격을 예측합니다. y_pred에는 모델의 예측값이 저장됩니다.
...
# 모델이 얼마나 잘 예측했는지 실제 정답(y_test)과 모델의 예측값(y_pred)을 비교하여 평가합니다.
#MAE (평균 절대 오차): 0.535
# ㆍ오차(실제값 - 예측값)의 절댓값에 대한 평균입니다.
# ㆍ해석: 모델의 예측값은 실제값과 평균적으로 약 0.535만큼 차이납니다. 타겟 변수의 단위가 10만 달러이므로, 약 $53,500 정도의 오차를 보인다고 해석할 수 있습니다.
#MSE (평균 제곱 오차): 0.538
# ㆍ오차를 제곱하여 평균을 낸 값입니다. 오차가 클수록 더 큰 페널티를 부여합니다. 값 자체보다는 다른 모델과 비교할 때 주로 사용됩니다.
#RMSE (평균 제곱근 오차): 0.734
# ㆍMSE에 제곱근을 씌운 값으로, MAE처럼 원래 데이터의 단위와 같아져 해석이 용이합니다.
# ㆍ해석: 예측 오차의 표준편차라고 볼 수 있으며, 약 $73,400의 오차 수준을 의미합니다.
#R² (결정 계수): 0.589
# ㆍ가장 중요한 평가지표 중 하나입니다. 모델이 데이터의 분산을 얼마나 잘 설명하는지를 나타냅니다. 1에 가까울수록 완벽한 예측을 의미합니다.
# ㆍ해석: 0.589는 이 모델이 주택 가격 변화(분산)의 약 **58.9%**를 설명한다는 뜻입니다. 즉, 나머지 약 41.1%는 이 모델이 사용한 8개 특성 외의 다른 요인에 의해 결정된다고 볼 수 있습니다. model.score()의 결과와 동일합니다.
...
# 이 선형 회귀 모델은 캘리포니아 주택 가격 변동의 약 **58.9%**를 설명할 수 있으며, 평균적으로 약 $53,500 ~ $73,400의 예측 오차를 가집니다.

# 이는 아주 나쁘지는 않지만, 모델의 성능을 더 개선할 여지가 많다는 것을 의미합니다. (예: 다른 회귀 모델 사용, 특성 공학 등)
