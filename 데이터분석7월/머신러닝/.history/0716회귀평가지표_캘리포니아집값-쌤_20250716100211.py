...
# • StandardScaler(): 각특성단위 통일하는 표준화도구. 각특성평균0, 표준편차1로 변환해, 소득(MedInc)처럼 값범위 큰 특성이 모델에 과도한 영향을 미치는 것을 방지
# • scaler.fit_transform(X_train): 훈련 데이터에 맞춰 스케일러를 학습(fit) 시키고, 그 규칙에 따라 데이터를 변환(transform) 합니다.지
