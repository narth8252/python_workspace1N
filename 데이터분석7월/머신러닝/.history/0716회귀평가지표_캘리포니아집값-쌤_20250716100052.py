...
# • StandardScaler(): 각 특성의 단위를 통일시켜주는 표준화 도구입니다. 각 특성의 평균을 0, 표준편차를 1로 변환하여, 소득(MedInc)처럼 값의 범위가 큰 특성이 모델에 과도한 영향을 미치는 것을 방지합니다.
# scaler.fit_transform(X_train): 훈련 데이터에 맞춰 스케일러를 학습(fit) 시키고, 그 규칙에 따라 데이터를 변환(transform) 합니다.
# scaler.transform(X_test): (중요!) 테스트 데이터는 훈련 데이터에서 학습된 스케일링 규칙을 그대로 적용하여 변환(transform)만 합니다. 이는 모델이 실전에서 처음 보는 데이터를 처리하는 상황을 시뮬레이션하기 위함입니다.
# ⚠️ 참고: 제공된 원본 코드의 scaler.fit_transform(X_test)는 테스트 데이터에 스케일러를 새로 학습시키는 것으로, 이는 데이터 누수(Data Leakage) 오류에 해당합니다. 훈련 세트의 정보(평균, 표준편차)로 테스트 세트를 변환해야 올바른 평가가 가능합니다.
