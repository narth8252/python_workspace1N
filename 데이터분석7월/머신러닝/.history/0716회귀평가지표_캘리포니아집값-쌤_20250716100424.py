...
# ⚠️참고: 제공된 원본코드의 scaler.fit_transform(X_test)는 테스트 데이터에 스케일러를 새로 학습시키는 것으로, 이는 데이터 누수(Data Leakage) 오류에 해당합니다. 훈련 세트의 정보(평균, 표준편차)로 테스트 세트를 변환해야 올바른 평가가 가능합니다.
