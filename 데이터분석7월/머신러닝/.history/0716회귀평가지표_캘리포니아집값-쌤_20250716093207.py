...
사이킷런(Scikit-learn)의 fetch_california_housing 데이터셋은 1990년 캘리포니아 인구 조사 데이터를 기반으로 하는 주택 가격 예측 회귀 모델 학습용 데이터셋입니다. 머신러닝, 특히 회귀 분석을 처음 접하는 입문자에게 적합한 데이터로 널리 사용됩니다.

이 데이터셋은 캘리포니아의 각 블록 그룹(최소 지리적 단위)에 대한 정보를 담고 있으며, 총 20,640개의 샘플과 8개의 특성(feature)으로 구성되어 있습니다.

주요 특성 (Features)
fetch_california_housing 데이터셋에 포함된 8개의 주요 특성은 다음과 같습니다.
...
이 데이터셋의 목표(target)는 해당 블록 그룹의 중간 주택 가격(MedHouseVal)을 예측하는 것입니다. 가격은 10만 달러 단위로 표시됩니다.
