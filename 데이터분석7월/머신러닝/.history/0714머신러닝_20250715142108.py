#0714 AM9:30 머신러닝 처리순서 정리. 자주읽어라.
"""
✅기초통계
현재상황을 정리하는것, 어떤 집단의 성격을 통계량 통해서 알아낸다.
평균, 분산, 표준편차, 중간값, 최빈값 등으로 알아낸다.
평균 = 모든요소의 합/전체데이터개수 ()
연평균 동해안바다온도 작년보다 높아짐- 50년치
평균만으로는 예측힘들더라- 올해가 작년보다 기온높은이유? 엘니뇨나 태풍이 많았다
오차 - 기대값(예상값)과 평균값의 차이의 합은 0
분산 - 오차의 제곱의 합, 절대값 안쓰고 제곱쓴 이유는 오차 커보
...
2. 각필드별 결측치 확인 
 ...
 9. 주성분분석
 10. 여러가지 모델로 학습하기, GridSearchCV사용도 가능 
