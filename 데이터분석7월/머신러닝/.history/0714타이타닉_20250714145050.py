...
# 2. 결측치 처리
# Age나이(실수형,float): 어린이와 여성우선원칙
# Fare요금(실수형,float): 비싼요금 1등석
# Embarked탑승항구(범주형,obj):S영국(대부분), C프랑스(1등석多), Q아일랜드
# Cabin선실(문자열,obj):BCDE는 1등석, NaN결측치많지만 앞글자만따면 유의미->상류층
for df in (train, test):
    df["Age"].fillna(df["Age"].mean(), inplace=True)  #연속형 → 평균
    df["Fare"].fillna(df["Fare"].median(), inplace=True)  #연속형 → 평균
    df["Embarked"].fillna(df["Embarked"].mean(), inplace=True)  #연속형 → 평균
    df["Cabin"].fillna(df["Cabin"].mean(), inplace=True)  #연속형 → 평균
