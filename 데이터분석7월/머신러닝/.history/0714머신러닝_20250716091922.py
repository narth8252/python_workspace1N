...
#기술발전이 빠르므로 바로바로배우는것보다 살짝 흥하려고하면 공부해야 따라간다. 너무빨리하면 흥하기전에 없어질수있다.
"""
✅기초통계
...
4. 평균값 중요한 경우
 ৹ 범주형자료(카테고리타입): 불연속적인 값, ex.연비등급(1~4등급)시 중간값은 중요하지않다.
                    ...
                    ex.100대의 차검사해서
                      등 급  1등급  2등급  3등급  4등급
                      차량수   5     10    60     25
                    각자 타겟일 경우도 있지만, 특성일수도 있다.
 ৹ 범주형 : 연비등급, 분할표, 히스토그램 - 구간별로 데이터 몇개인지, 차이검정
 ৹ 비범주형(연속형) - 연비자체, 평균, 표준편차, 중간값, 차이검정

  • 차이검정: 2명이 갖고있는 책종류를 따져봄
                      A    B     - 둘 사이에 차이가 있는지?
        책(권) 웹 소설 100  120     객관화 방법, 카이제곱검증
              추리소설 400  380     R언어
              자연주의 50    0      차이가 있긴한데 본질적인차이인지?
              신비주의 10    15     유의수준0.05미만이어야 차이인정
  • 정규분포가 있으면 농구선수 키가 2m7cm인데 전체데이터중에서 2m7cm일 확률이 0.05미만이어야 과학적으로 차이인정
  • 범주형 데이터는 카이제곱검증
  • 비범주형데이터는 T-test검증
  그룹2개 만들어서 한그룹은 신약, 다른그룹은 가짜약줘도 결과가 나오기때문에 신약은 유의수준0.05미만이어야 효과인정받음

  • 대립가설 - 현재가설을 폐기할 가설(한국남자평큔키가 173cm보다 작다 또는 크다)
  • 귀무가설 - 현재상태(한국남자평균키173cm)


✅추론통계
기초통계를 바탕으로 예측할때(머신러닝,딥러닝)
1. 머신러닝
 1)지도 학습: 라벨있을때, 타겟있을때
 2)비지도학습: 라벨없을때, 타겟없을때, 보통은 지도학습 전단계로 특성공학 담당.
            새로운 특성만들거나, 차원축소하거나, 연관성찾아보거나, 스케일링 등
 3)강화 학습: 알파고, 게임알고리즙에게 당근과 채찍을 줌. 
            챗GPT하지마라. 못한다. 파이썬3.9이하까지만 지원중.
            할사람은 반드시 별도의 가상환경 만들어야한다.
            dfs(깊이우선탐색), bfs(너비우선탐색) - 둘다 안쓴다.
...
3. 이상치제거 
  boxplot이 시각화에 젤추천 : 맨아래,맨위에 점동그라미 있으면 문제있다고 값버림.
  처리방식 중 IQR방식 많이사용하나, 내장함수없어서 직접만들어야함.

4. 중복값제거

5. 데이터 자체가 잘못들어온 값(여기가 젤힘들고 에러많이나니 추적해서 잡아야함)
    value_counts 함수나 Unique로 체크하기 
    값바꾸기를 시도하거나 행을 삭제 

6. 라벨 또는 원핫인코딩
 • get_dummies함수
 • 라벨링(수동) - 성별(male, female)정보가 문자열로 들어오면 연산불가능하니 male=0, female=1이라고 바꾼다.
          이처럼 값범위가 4개정도일때는 할만함
 • 원핫인코딩(자동) - 수동할수있으나 자동많이함
           하지만 라벨링 범위 큰경우(직업 등)
            직업에 라벨링 매기면 1보다는 15가 큰수가 중요하다고 판단할수 있어서 가중치 구하면 결과가 왜곡된다.
            그래서 직업1, 직업1, 직업3, 직업4 이런식으로 라벨링

7. 스케일링: 연산이니까 값의 범위가 너무차이나면 큰값기준으로 가중치 결정
 y= w1x1 + w2x2 + w3x3 +....+xnxn +b
    w1, w2, w3, ...., b 계수/가중치라고 함
    가중치에 영향많이 미치는 대표적인 예: 서포트벡터머신, 딥러닝:단위가 영향을 미치는 알고리즘
    스케일을 맞춰준다(정규화, 표준화랑 비슷)
...
12. 평가지표
  회귀와 분류인 경우로 나뉜다
 1)회귀
 mae : Mean Absolute Error 평균절대오차, |기대값-실제값|/개수, 이상치에 덜민감
 mse : Mean Squared Error  평균제곱오차, (기다값-실제값)^2제곱/개수, 이상치에 더민감 
      딥러닝에서 손실함수(오차계산시 많이 쓰임)
 rmse: Root Mean Squared Error - mse의 제곱근
 R2 : 결정계수, 현재 회귀계열에서 score함수의 결과값
      1 예측굿, 0으로 갈수록 별로, 음수면 심각
      취약점: 독립변수(특성)의 수가 증가하면 예측력 높지않은데, 이 값이 좋아지는 경우 있다.
      업그레이드버전 Adjusted R-squared : R언어가 이걸쓴다
 mape: Mean Absolute Percentage Error 퍼센트로 따짐
 등
2)분류
2-1)특징: 데이터의 불균형성때문에 제대로 예측불가
      ex.암은 예측정확성만으로 좋은모델이라고 평가가능한가?
      반대의경우 iris데이터의 경우 균등데이터셋(라벨기준으로)
      iris가 종을 못맞춰도 그러려니
      암환자 음성을 양성으로 예측시 재검, 양성을 음성으로 예측시 큰일남.
      데이터의 불균등이 너무 심해서 아무거나 하나로 대답함. 그경우도 정확도만 가지고 파악어려움
      정확도: 맞춘개수/전체개수, 양성3%, 음성97%
      이때 전부 음성이라고 해도 97%정확도 보여서 별도의 평가지표 가져야한다.
2-2)오차행렬: confusion Matrix 대각선이 정상
실제값 | Positive | Negative | 예측값 ---------------- 
Positive | TP | FP | Negative | FN | TN |
TP (True Positive): 실제 Positive인 경우를 Positive로 맞춘 수
FP (False Positive): 실제 Negative인 경우를 Positive로 잘못 예측한 수
FN (False Negative): 실제 Positive인 경우를 Negative로 잘못 예측한 수
TN (True Negative): 실제 Negative인 경우를 Negative로 맞춘 수
암환자셋 양성0, 음성1, cofution Matrix에 맞게 만드려면 양성1,음성0으로 바꿔분석
• 정확도: 맞게 예측한 비율, 불균형데이터셋일때, 위험한상태 예측의 경우 정확도가 도움안됨
        분류모델들의 score함수는 정확도를 말한다
        아주 운나쁜경우 한개만 대답해도 정확도 높을수있다.
• 정밀도(precision): 모델이 양성으로 예측했는데 실제양성으로 예측한비율  
                  스팸메일이 아닌데 스팸으로 예측해 스팸함으로 보내면 문제됨
• ⭐재현율(recall): 민감도 실제양성들중 모델이 양성으로 예측한것들
                FN을 줄이고자 할때 중요, 암환자
• f1-score: 정밀도와 재현율의 조화평균
재현율 보기위한 시각차트 ROC curve
auc : ROC
