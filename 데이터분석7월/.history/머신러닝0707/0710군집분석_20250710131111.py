...
결과: "정답(label)"이 없기 때문에 모델이 자동으로 그룹을 찾아냄.

입력: 데이터만 있으면 됨 (라벨 불필요).

출력: 각 데이터가 어느 군집(cluster)에 속하는지.

군집 수(K): 알고리즘에 따라 지정하거나 추정해야 함. 예: KMeans(n_clusters=3)

활용: 고객 세분화, 문서 분류, 이상 탐지 등.
----------------------------------------------
