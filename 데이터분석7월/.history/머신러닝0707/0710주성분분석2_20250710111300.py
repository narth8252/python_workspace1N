...
0710 am11시 
PCA(주성분분석) 차원 축소 정리
1. 개념 및 필요성*PCA(Principal Component Analysis, 주성분분석)**는 특성(Feature)이 많은 데이터에서, 원래의 특성들로부터 새로운 특성(주성분)을 생성하여 차원을 축소하는 방법입니다.
차원 축소는 데이터의 복잡성을 줄이고, 데이터 시각화 및 모델 성능 개선에 도움을 줍니다.
2. 다중공선성과 회귀분석과의 관계
다중공선성은 여러 특성(X)들 사이에 상관관계(연관성)가 높을 때 발생합니다. 예를 들어, 유사한 댐의 수위 데이터(A1, A2, B1, C1 등)를 모두 예측에 사용할 때 다중공선성 문제가 생길 수 있습니다.
회귀분석
