...
for i in range(30): #특성(feature) 30개
    ...
    #bins구간리스트                           
    ax[i].hits(malignant[:, i], bins=bins, color='purple', alpha=0.5) #악성데이터 보라색으로
    ax[i].hits(benign[:, i], bins=bins, color='green', alpha=0.5) #양성데이터 녹색으로

    #제목
    ax[i].set_title(cancer['feature_names'], bins=bins, color='green', alpha=0.5) #양성데이터 녹색으로
