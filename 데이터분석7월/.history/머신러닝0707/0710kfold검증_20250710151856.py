...
ㆍ이 과정을 k번 반복하면 총 k개의 성능 평가 결과가 나옵니다. 
ㆍ이 k개의 결과의 평균을 내어 모델의 최종 성능으로 삼습니다.
...
3. 단점
ㆍ계산 비용 증가: 모델을 k번 훈련하고 평가해야 하므로, 한 번만 훈련하는 것보다 계산 비용과 시간이 더 많이 소요됩니다. k값이 커질수록 이 단점은 더욱 부각됩니다.
ㆍk값의 선택
...
k값이 너무 크면 (예: 데이터 포인트의 수와 같게, Leave-One-Out Cross-Validation), 
훈련 세트가 전체 데이터와 거의 유사해져 평가 결과의 편향은 줄어들지만, 
계산 비용이 매우 커지고 각 훈련 세트가 거의 동일하여 평가 결과의 분산이 커질 수 있습니다.
...
ㆍㆍKFold는 데이터를 나눠가며 학습과 테스트를 여러 번 수행 → 평균 성능 추정
KFold, cross_val_score, StratifiedKFold 등 도구 다양
분류 문제면 보통 StratifiedKFold가 더 안정적임
"""
