...
일반화 오류를 추정하기 위해 사용되는 통계적 방법
데이터를 훈련/테스트 세트로 한 번만 나누는 것보다 더 신뢰성 높은 모델 평가.
데이터를 K개의 부분(fold)**으로 나눠서, 1개는 테스트용, 나머지 K-1개는 학습용으로 사용.
이 과정을 K번 반복해서, 모든 데이터가 한 번씩 테스트셋이 되게 함.
마지막에 K개의 성능 평균을 구해 최종 평가로 사용.

k-겹 교차 검증은 전체 데이터를 **k개의 동일한 크기의 부분 집합(fold)**으로 나눕니다. 
그런 다음 아래 과정을 k번 반복합니다.
