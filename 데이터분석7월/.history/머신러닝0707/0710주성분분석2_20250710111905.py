...
6. 결론
특성이 많거나 다중공선성 문제가 있을 때 PCA를 통해 성능 향상, 시각화, 노이즈 제거, 과적합 방지 등 다양한 효과를 기대할 수 있습니다.
fit, transform 과정을 거쳐 주성분 기반의 데이터를 얻어 학습에 활용합니다.
----------------------------------------------
...
pca = PCA(n_components=2)   # 주성분 2개로 차원 축소
pca.fit(X_train)            # 학습
X_train_pca = pca.transform(X_train)    # 변환