...
#2.히트맵그리기
# 2x2 상관계수 행렬을 히트맵으로 시각화하며, 각 셀의 값은 소수점 두 자리까지 보여줍니다.

# 실제 분석에 맞는 데이터 프레임의 .corr() 함수 등으로 생성 가능
correlation_matrix = [[1.0, 0.5], [0.5, 1.0]] 

# 히트맵 속성 설정
annot=True #차트 줄속성, 셀안에 값표기여부(True:표시)
cmap = 'coolwarm' #히트맵에서 가장많이 사용하는색상(양은 빨강, 음은 파랑으로 짙어짐)
ftm ='.2f' #셀에 표시될 숫자의 소수점자리수 (여기서는 소수점 둘째 자리)
...
# cmap=cmap:색상맵(컬러 팔레트)을 지정합니다. 예: 'viridis', 'coolwarm', 'Blues' 등
# fmt=ftm: 셀안 숫자형식 지정.'.2f'로 설정하면 소수점 둘째 자리까지 표기
# 통상적으로 fmt로 작성하며 변수명 확인이 필요할 수 있습니다.
# linewidths=.5: 셀사이 경계선 두께를 0.5로 설정
# 변수 annot, cmap, ftm는 코드 상단이나 앞서서 정의되어 있어야 오류가 발생하지 않습니다.
# 값이 표시되는 포맷(fmt)이나 색상맵(cmap)은 원하는 대로 변경 가능합니다.

# x축, y축 레이블 회전 및 레이아웃 조정
plt.xticks(rotation=45, ha='right')  # x축 라벨을 45도로 회전시켜 가독성을 높입니다.
