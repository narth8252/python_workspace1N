...
머신러닝에서 모델의 일반화 성능을 평가하는 대표적인 방법
 머신러닝 모델의 성능을 평가하고 일반화 오류를 추정하기 위해 사용되는 통계적 방법입니다.
   데이터를 훈련 세트와 테스트 세트로 한 번만 나누는 것보다 더 신뢰성 높은 모델 평가를 제공합니다.
...
#2.데이터를 나누기: 한번저장해서 쓰는 딕셔너리 방식 접근 Vs 매번 data[...]로 꺼내야 함
X = data['data']   #ndarray 2차원배열
y = data['target'] #ndarray 1차원배열 

# 데이터 로딩: 속성 직접 접근: 더 간단하고 직관적 Vs 구조를 파악하기엔 어려움
X = load_iris().data
y = load_iris().target
