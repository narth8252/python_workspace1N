...
# nltk.download('punkt') #토큰나이저 다운코드 안되면 아래로. ver문제
...
token_en = word_tokenize(c)
print(token_en)

data = Counter(token_en) #각단어개수를 센다
print(data) #잘모르겠으면 출력해봐. dict타입으로 숫자세줌
# ({'happy': 1, 'End': 1})
...
temp_data = dict(data)
#이미지가 넘파이형태의 배열로 전달
mask_image = np.array(Image.open("./data1/image/stormtrooper_mask.png"))

#generate_from_frequencies
fontpath = "C:\Windows\Fonts\malgun.ttf"
wordcloud = WordCloud(font_path=fontpath, mask=m).generate_from_frequencies(temp_data)
