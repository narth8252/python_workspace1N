...
model = RandomForestClassifier(random_state=0, n_estimators=100, max_depth=4)
...
# 트리를 랜덤으로 계속만들어내서 측정값달라지므로 
#                               0으로 고정필수,만들트리의 최대개수, 트리최대깊이 지정,  학습속도
model = GradientBoostingRegressor(random_state=0, n_estimators=300, max_depth=4, learning_rate=0.1)
model.fit(X_train, y_train)     # 학습을 하고
y_pred = model.predict(X_test)
