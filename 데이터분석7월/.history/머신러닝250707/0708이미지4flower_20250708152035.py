...
#폴더이름 리스트를 가져와 정렬
    # 폴더 이름 리스트를 가져와 정렬합니다.
    folder_names = sorted(os.listdir(main_folder_path))
...
#함수실행
X_data, y_labels = label_images_from_folders(data_path)

#결과확인
print("\n--- 데이터 라벨링 결과 ---")
print(f"입력데이터(X)형태: {X_data.shape}")
print(f"입력데이터(y)형태: {y_data.shape}")
