...
# 1. 전체 사업자번호 추출 (000-00-00000 형식)
pattern = r"\d{3}-\d{2}-\d{5}"
all_numbers = re.findall(pattern, contents)
print("전체 사업자번호:", all_numbers)

# 2. 개인사업자(가운데 2자리가 90~99)만 추출
pattern_personal = r"\d{3}-(9[0-9])-\d{5}"
personal_numbers = re.findall(pattern_personal, contents)
personal_full_numbers = re.findall(r"\d{3}-(9[0-9])-\d{5}", contents)
# 위의 findall은 그룹만 반환하므로, 전체 번호를 추출하려면 아래처럼 사용
personal_full_numbers = re.findall(r"\d{3}-9[0-9]-\d{5}", contents)
print("개인사업자(90~99) 번호:", personal_full_numbers)
contents = """
    우리커피숍 100-90-12345
    영풍문고 101-91-12121
    영미청과 102-92-23451
