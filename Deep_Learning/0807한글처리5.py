...
# C:\Users\Admin\Documents\GitHub\python_workspace1N\Deep_Learning\data\cc.ko.300
#cc.ko.300.vec
#기학습된 자료 사용 _word2vec_
...
def study():
    ...
    embedding_dim = 300 #대충씀. 특별히 사전에 학습된 내용을 사용하는게 아니면 차원은 내맘대로

    korean_word2vec_path="./data/cc.ko.300.vec" #기학습된 파일을 불러와서 학습할것이다.
    korean_word_vector = KeyedVectors.load_word2vec_format(korean_word2vec_path, binary=False,
                                                           encoding="utf-8", limit=50000)
    #txt파일은 binary=False. True로 해도    korean_word2vec_path = "./data/cc.ko.300.vec" 읽히긴함
    print(korean_word_vector.vector_size)
    embedding_dim = korean_word_vector.vector_size

    #임베딩행렬 구축
    embedding_matrix = np.zeros((voca_size, embedding_dim)) #np.zeros함수가 매개변수를 tuple로 받아서 괄호필요
    #2차원배열 만들기: 
    #자연어를 숫자로 0.2, 01, 0.2.... 감동 0.1, ....
    for i, word in enumerate(vocabulary):
        if i < voca_size: #max_tokens벗어나지 않는 범위내에서
            try:
                embedding_vector = korean_word_vector[word] #파일에있는 벡터값 읽어와서
    ...
    x = layers.Embedding(
        ...
        embeddings_initializer= keras.initializers.Constant(embedding_matrix), #초기값을 랜덤 → 학습된데이터
        trainable = False, #학습된데이터에서 현재 들고온 가중치 사용
        mask_zero=True
    )(inputs)
...
# (mytensorflow) C:\Users\Admin\Documents\GitHub\python_workspace1N\Deep_Learning>python 0807한글처리5.py
# 2025-08-07 16:36:40.502645: I tensorflow/core/util/port.cc:153] oneDNN custom operations are on. You may see slightly different numerical results due to floating-point round-off errors from different computation orders. To turn them off, set the environment variable `TF_ENABLE_ONEDNN_OPTS=0`.
# 2025-08-07 16:36:45.079952: I tensorflow/core/util/port.cc:153] oneDNN custom operations are on. You may see slightly different numerical results due to floating-point round-off errors from different computation orders. To turn them off, set the environment variable `TF_ENABLE_ONEDNN_OPTS=0`.
...
# [Korpora] Corpus `nsmc` is already installed at C:\Users\Admin\Korpora\nsmc\ratings_train.txt
# [Korpora] Corpus `nsmc` is already installed at C:\Users\Admin\Korpora\nsmc\ratings_test.txt
# LabeledSentence(text=('아 더빙.. 진짜 짜증나네요 목소리', '흠...포스터보고 초딩영화줄....오버연기조차 가볍지 않구나', '너무재밓었다그 래서보는것을추천한다'), label=[0, 1, 0])
# Found 2000 files belonging to 2 classes.
# 2025-08-07 16:36:58.801020: I tensorflow/core/platform/cpu_feature_guard.cc:210] This TensorFlow binary is optimized to use available CPU instructions in performance-critical operations.
# To enable the following instructions: SSE3 SSE4.1 SSE4.2 AVX AVX2 FMA, in other operations, rebuild TensorFlow with the appropriate compiler flags.
# Found 2000 files belonging to 2 classes.
# Found 50000 files belonging to 2 classes.
# --- 전처리 완료 ---
# 2025-08-07 16:37:41.594311: I tensorflow/core/framework/local_rendezvous.cc:405] Local rendezvous is aborting with status: OUT_OF_RANGE: End of sequence
# ['', '[UNK]', '이', '영화', '의', '에', '가', '을', '를', '들', '도', '는', '정말', '은', '한', '최고', '적', '너무', '다', '잘']
# (32, 20)
# 2025-08-07 16:37:43.804985: W tensorflow/core/kernels/data/cache_dataset_ops.cc:914] The calling iterator did not fully read the dataset being cached. In order to avoid unexpected truncation of the dataset, the partially cached contents of the dataset  will be discarded. This can happen if you have an input pipeline similar to `dataset.cache().take(k).repeat()`. You should use `dataset.take(k).cache().repeat()` instead.
# (32, 1)
# [  12  168    8   25  162   29    7 1565   82  162]
# 정말 이영화 를 보고 많은 생각 을 하게되 고 많은
# 2025-08-07 16:37:44.634269: I tensorflow/core/framework/local_rendezvous.cc:405] Local rendezvous is aborting with status: OUT_OF_RANGE: End of sequence
# --- 최종 데이터셋 준비작업 완료 ---
# 300
# "Key '' not present"
# "Key '[UNK]' not present"
...
# "Key '재밌고' not present"
# ...
# "Key '123초' not present"
# "Key '123412341234' not present"
# "Key '10403' not present"
# Model: "functional"
# ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
...
# ├───────────────────────────────┼───────────────────────────┼─────────────────┼────────────────────────────┤
# │ embedding (Embedding)         │ (None, None, 300)         │       1,328,400 │ input_layer[0][0]          │
# ├───────────────────────────────┼───────────────────────────┼─────────────────┼────────────────────────────┤
...
# └───────────────────────────────┴───────────────────────────┴─────────────────┴────────────────────────────┘
#  Total params: 1,413,713 (5.39 MB)
#  Trainable params: 85,313 (333.25 KB)
#  Non-trainable params: 1,328,400 (5.07 MB)

#  --- 모델 훈련 시작 ---
# Epoch 1/5
# 2025-08-07 16:38:49.911359: E tensorflow/core/util/util.cc:131] oneDNN supports DT_BOOL only on platforms with AVX-512. Falling back to the default Eigen-based implementation if present.
# 63/63 ━━━━━━━━━━━━━━━━━━━━ 55s 595ms/step - accuracy: 0.5085 - loss: 0.6956 - val_accuracy: 0.5000 - val_loss: 0.6936
# Epoch 2/5
# 63/63 ━━━━━━━━━━━━━━━━━━━━ 4s 60ms/step - accuracy: 0.5055 - loss: 0.6960 - val_accuracy: 0.5000 - val_loss: 0.6935
# Epoch 3/5
# 63/63 ━━━━━━━━━━━━━━━━━━━━ 4s 60ms/step - accuracy: 0.5026 - loss: 0.6950 - val_accuracy: 0.5000 - val_loss: 0.6934
# Epoch 4/5
# 63/63 ━━━━━━━━━━━━━━━━━━━━ 4s 48ms/step - accuracy: 0.5025 - loss: 0.6946 - val_accuracy: 0.5000 - val_loss: 0.6934
# Epoch 5/5
# 63/63 ━━━━━━━━━━━━━━━━━━━━ 4s 67ms/step - accuracy: 0.4910 - loss: 0.6957 - val_accuracy: 0.5000 - val_loss: 0.6934
# --- korean_rnn_sentiment_히스토리 저장 완료 ---
# 1/1 ━━━━━━━━━━━━━━━━━━━━ 2s 2s/step
# 이 영화는 정말 감동적이었어요. ===> 부정 0.4857903
# 영화수준이 너무 낮아요. ===> 부정 0.49085188
# 반드시 봐야할 영화입니다. ===> 부정 0.48687267
# 배우 아니면 이 영화를 안봤을 거예요. ===> 부정 0.49401242