...
# 값의 개수와 인덱스의 개수를 # ...existing code...
ts = pd.Series(np.random.randn(30), 
    index=pd.date_range(start="2025-07-04", periods=30, freq="T"))
print(ts)
# ...existing code...맞추세요.
# 예시 1) 12개 데이터만 생성:
np.random.seed(0)# ...existing code...
ts = pd.Series(np.random.randn(30), 
    index=pd.date_range(start="2025-07-04", periods=30, freq="T"))
print(ts)
# ...existing code...
ts = pd.Series(np.random.randn(12), 
    index=pd.date_range(start="2025-07-04", periods=12, freq="T"))
print(ts)


#period필드=기간 와freq프리퀀시=D 둘이 결합해서 일,시간
d = pd.data_range(start="2025-07-04", periods=30, freq='T')
print(d)

d = pd.data_range(start="2025-07-04", periods=30, freq='W')
print(d)
...
#->이게 바로 정규분포. 여기서 출발
np.random.seed(0) #랜덤값이 일정하게
ts = pd.Series( np.random.randn(12),
                index=pd.date_range(start="2025-0101", periods=12, freq="ME"))
#12개의 가짜데이터를 만들었음.
#날짜가 인덱스임.
